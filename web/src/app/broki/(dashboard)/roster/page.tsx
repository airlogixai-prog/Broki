...
import { CalendarBlank, Users, UploadSimple, ArrowsClockwise } from "@phosphor-icons/react";
...
export default function RosterPage() {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      formData.append("type", "roster_pdf");
      await brokiApi.rosterUpload(formData);
      alert("Horario enviado correctamente.");
    } catch (e) {
      console.error("[RosterPage] upload error", e);
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Sube el PDF del horario para actualizar los datos en n8n.
            </p>
          </div>
        </div>
        <input
        ...
        <button
          ...
          disabled={uploading}
          className="flex items-center gap-2 px-4 py-2 text-sm font-bold bg-brand-600 text-white rounded-lg hover:bg-brand-700 disabled:opacity-60 transition-colors"
        >
          {uploading ? (
